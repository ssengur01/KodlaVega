...
namespace InvoiceDocumentService.API.Controllers
{
    ...
    [ApiController]
    public class AlFatHareketsController : ControllerBase
    {
        private readonly IMediator _mediator;

        public AlFatHareketsController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        [HttpGet]
        public async Task<IActionResult> GetAllAlFatHarekets()
        {
            var query = new GetAllAlFatHareketsQuery();
            var result = await _mediator.Send(query);
            return Ok(result);
        }

        [HttpGet("{id}")]
        public async Task<IActionResult> GetAlFatHareketById(int id)
        {
            var query = new GetAlFatHareketByIdQuery { Ind = id };
            var result = await _mediator.Send(query);
        ...
        public async Task<IActionResult> GetByEvrakNo(int evrakNo)
        {
            ...
            var query = new GetAlFatHareketByEvrakNoQuery { EvrakNo = evrakNo };
            var result = await _mediator.Send(query);
