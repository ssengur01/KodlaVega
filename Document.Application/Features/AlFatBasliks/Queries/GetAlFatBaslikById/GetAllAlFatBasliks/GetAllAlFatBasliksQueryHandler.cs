...
{
    public class GetAllAlFatBasliksQueryHandler : IRequestHandler<GetAllAlFatBasliksQuery, List<AlFatBaslikDto>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetAllAlFatBasliksQueryHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        public async Task<List<AlFatBaslikDto>> Handle(GetAllAlFatBasliksQuery request, CancellationToken cancellationToken)
        {
            var alFatBasliks = await _unitOfWork.AlFatBaslikRepository.GetAllAsync();

          
            var dtoList = alFatBasliks.Select(alFatBaslik => new AlFatBaslikDto(alFatBaslik)
            {
                IND = alFatBaslik.IND,
